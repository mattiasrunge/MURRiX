...
class Connections extends Component {
    ...
    async onConnect(id, person) {
        ...
        try {
            if (this.state.persons[id]) {
                await api.vfs.unlink(`${this.props.node.path}/tags/${id}`);
            }

            if (person) {
                await api.vfs.symlink(person.path, `${this.props.node.path}/tags/${id}`);
            }
    ...
    onRemoveFace = async (face) => {
        ...
        try {
            if (this.state.persons[face.id]) {
                await api.vfs.unlink(`${this.props.node.path}/tags/${face.id}`);
            }
    ...
    onPersonAdd = async (person) => {
        ...
        try {
            await api.vfs.symlink(person.path, `${this.props.node.path}/tags`);
    ...
    onRemovePerson = async (person) => {
        ...
        try {
            await api.vfs.unlink(person.extra.linkPath);
