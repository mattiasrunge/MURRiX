...
function selfManager(murrix)
{
  ...
  {
    exports.runCommand(command, args, options, callback);
  };
  ...
  {
    return exports.inArray(needle, haystack);
  };
  ...
  {
    return exports.isDaylightSavings(datestring);
  };

  self.timestamp = function(value)
  {
    return exports.timestamp(value);
  };

  self.parseTimezone = function(string)
  {
    return exports.parseTimezone(string);
  };

  self.cleanDatestring = function(datestring)
  {
    return exports.cleanDatestring(datestring);
  };

  self.md5File = function(filePath, callback)
  {
    return exports.md5File(filePath, callback);
  };
  ...
  self.intval = function(value)
  {
    ...
    {
      console.error("Could not convert value to int: \"" + value + "\" (" + (typeof value) + ") -> \"" + intvalue + "\" (" + typeof intvalue + ")");
      intvalue = 0;
    }
  ...
  self.getTemplateFile = function(filename, callback)
  {
    return exports.getTemplateFile(filename, callback);
  };
...
exports.Manager = selfManager;





exports.getConfiguration = function(configFile)
{
  var config = {};

  config.databaseHost = "localhost";
  config.databasePort = 27017;
  config.databaseName = "murrix";
  config.httpPort = 8080;
  config.filesPath = "./files/";
  config.mediaCachePath = "./cache/";
  config.sessionName = "murrix";

  if (fs.existsSync(configFile))
  {
    console.log("Found user configuration file at " + configFile);

    var userConfig = require(configFile);

    for (var n in userConfig)
    {
      config[n] = userConfig[n];
    }
  }
  else
  {
    console.log("No user configuration found, using default parameters");
  }
