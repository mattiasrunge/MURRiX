...
function MurrixUtilsManager(murrix)
{
  ...
  {
    if ((!b.when || b.when.timestamp === false) && (!a.when || a.when === false))
    {
      if (!b.name || !a.name)
      {
        return 0;
    ...
    else if ((b.when && b.when.timestamp !== false) && (!a.when || a.when === false))
    {
      return -1;
    }
    else if ((a.when && a.when.timestamp !== false) && (!b.when || b.when === false))
    {
      return 1;
    }
