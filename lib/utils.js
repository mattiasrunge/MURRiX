...
exports.getPreview = function(session, nodeManager, id, width, height, square, callback)
{
  ...
  fs.exists(filename, function(exists)
  {
    ...
    nodeManager.findOne(session, { _id: id }, "items", function(error, itemData)
    {
      ...
      if (!itemData.specific)
      {
        callback("Missing specific information from item!");
        return;
      }
      ...
      convertOptions.originalHeight = itemData.specific.exif.ImageHeight;
      convertOptions.originalWidth = itemData.specific.exif.ImageWidth;
      convertOptions.square = square;
      convertOptions.mirror = itemData.specific.mirror;
      convertOptions.angle = itemData.specific.angle;
      ...
      switch (itemData.specific.exif.MIMEType)
      {
        case "video/mpeg":
        case "video/avi":
        ...
        case "video/3gpp":
        {
          ...
          if (itemData.specific.exif.Compression && itemData.specific.exif.Compression === "dvsd")
          {
      ...
      callback("Can not create preview for MIMEType " + itemData.specific.exif.MIMEType);
    });
  });
};
