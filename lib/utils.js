...
function MurrixUtilsManager(murrix)
{
  ...
  self.rawImageMimeTypes = [ "image/x-canon-cr2", "image/x-raw", "image/x-canon-crw", "image/x-nikon-nef", "image/CR2", "image/x-panasonic-raw2" ];
  self.imageMimeTypes = [ "image/jpeg", "image/gif", "image/tiff", "image/png", "image/bmp" ].concat(self.rawImageMimeTypes);
  self.videoMimeTypes = [ "video/mpeg", "video/avi", "video/quicktime", "video/x-ms-wmv", "video/mp4", "video/3gpp", "video/x-msvideo", "video/vnd.dlna.mpeg-tts", "video/m2ts" ];
  ...
  self.itemReadExif = function(itemData) {
  ...
  self.readExif = function(filename, callback)
  {
    fs.exists(filename, function(exists)
    {
      ...
      self.run(self.exiftoolPath, args, {}, function(error, output)
      {
        ...
        try
        {
          var exif = JSON.parse(output.replace(/[\x00-\x1F\x7F-\x9F]/g, ""))[0];
