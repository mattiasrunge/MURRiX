  ...
  fs.exists(filename, function(exists)
  {
    ...
    nodeManager.findOne(session, { _id: id }, "items", function(error, itemData)
    {
      ...
      switch (itemData.exif.MIMEType)
      {
        ...
        case "audio/mpeg":
        {
          var tempfile1 = "/tmp/audioconvert" + (new Date().getTime()) + id + ".wav";
          var tempfile2 = "/tmp/audioconvert" + (new Date().getTime()) + id + ".png";
          var args = [];
          ...
          MurrixUtils.runCommand(mpg123Path, args, {}, function(error, output)
          {
            ...
            generateWaveform(tempfile1, tempfile2, options, function(error)
            {
              MurrixUtils.removeFile(tempfile1);
              ...
              createPreview(tempfile2, filename, convertOptions, function(error)
              {
                MurrixUtils.removeFile(tempfile2);
    ...
    nodeManager.findOne(exports._videoQueue[0].session, { _id: exports._videoQueue[0].id }, "items", function(error, itemData)
    {
      ...
      switch (itemData.exif.MIMEType)
      {
        ...
        case "video/x-msvideo":
        {
          ...
          var tempfile2 = "/tmp/videogenerate" + (new Date().getTime()) + exports._videoQueue[0].id + "_2";
          var args = [];
  ...
  fs.readdir(dir, function(error, list)
  {
    ...
    list.forEach(function(file)
    {
      if (file.indexOf(id) !== -1)
      {
        ...
        MurrixUtils.removeFile(dir + "/" + file);
      }
