...
function MurrixConfigurationManager(murrix, configFile)
{
  ...
  murrix.on("init", function()
  {
    fs.exists(configFile, function(exists)
    {
      ...
      var folders = [ self.getPathFiles(), self.getPathCache(), self.getPathTemp(), self.getPathSessions() ];
      var folderChain = new Chain();

      folderChain.addMany(folders, function(path, options, callback)
      {
        murrix.utils.file.createDirectory(path, function(error)
        {
          if (error)
          {
      ...
      folderChain.run(function(error)
      {
        if (error)
        {
  ...
  self.getPathSessions = function()
  {
    return path.resolve(murrix.basePath(), self.sessionPath) + "/";
  };
