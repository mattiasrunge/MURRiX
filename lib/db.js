...
function MurrixDatabaseManager(murrix)
{
    ...
    self.mongoDb.collection(collectionName, function(error, collection)
    {
      ...
      if (data._id)
      {
        self.findOne({ _id: data._id }, collectionName, function(error, dataOld)
        {
          ...
          murrix.triggers.trigger(triggerEvent, dataOld, data, function(error, dataNew)
          {
            if (error)
            {
              callback(error);
      ...
      else
      {
        ...
        murrix.triggers.trigger(triggerEvent, {}, data, function(error, dataNew)
        {
          if (error)
          {
            callback(error);
  ...
  self.items.hideRaw = function(session, itemDataRaw, callback)
  {
    ...
    self.findOne(session, query, "items", function(error, itemData)
    {
      if (error)
      {
        callback(error);
