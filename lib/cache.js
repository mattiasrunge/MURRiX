...
function MurrixCacheManager(murrix)
{
  ...
  self.imageMimeTypes = [ "image/x-canon-cr2", "image/jpeg", "image/gif", "image/tiff", "image/png", "image/bmp", "image/x-raw", "image/x-canon-crw" ];
        ...
        job.chain.final((function(job)
        {
          return function(options)
          {
            ...
            removeChain.final(function(options2)
            {
              ...
              murrix.emit("cacheJobDone", { id: path.basename(job.options.targetFilename) });
    ...
    fs.readdir(self.path, function(error, list)
    {
      ...
      for (var n = 0; n < list.length; n++)
      {
        ...
        chain.add((function(filename)
        {
          ...
          {
            murrix.logger.debug(self.name, "Will remove " + self.path + filename + "...");

            murrix.utils.removeFile(self.path + filename, function(error, existed)
            {
              if (error)
              {
                murrix.logger.error(self.name, "Failed to remove file, reason: " + error);
              ...
              if (existed)
              {
                murrix.logger.debug(self.name, "Removed " + self.path + filename + "!");
              }
              else
              {
                murrix.logger.debug(self.name, self.path + filename + " never existed!");
              }
