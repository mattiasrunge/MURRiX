...
function MurrixCacheManager(murrix)
{
  ...
  murrix.on("configurationLoaded", function()
  {
    ...
    self.path = "../" + murrix.config.cachePath || self.path;
    self.tempPath = murrix.config.tempPath || self.tempPath;
  });
  ...
  self._getDirectory = function()
  {
    return path.resolve(__dirname, self.path);
  };


  fs.exists(self._getDirectory(), function(exists)
  {
    if (exists)
    {
      return;
    ...
    fs.mkdir(self._getDirectory(), 0770, function(error)
    {
      if (error)
      {
      ...
      murrix.logger.info(self.name, "Created cache directory at " + self._getDirectory());
    });
  });
    ...
    for (var n = 0; n < self.runningJobs.length; n++)
    {
      ...
      if (!job.started)
      {
        ...
        job.chain.final(function(options)
        {
          ...
          {
            murrix.utils.removeFile(job.options.temporaryFilenames[i]);
          }
  ...
  self.queueItem = function(id, options)
  {
    ...
    {
      options.targetFilename = self._getDirectory() + "/" + id + "_" + options.width + "x" + options.height + "_"  + options.square + ".jpg";
    }
    else if (options.type === "video")
    {
      options.targetFilename = self._getDirectory() + "/" + id + ".webm";
    }
    else
    {
    ...
    self._findItem(id, function(error, itemData)
    {
      ...
      self._jobExists(options.targetFilename, function(exists)
      {
        ...
        options.id = id;
        options.sourceFilename = path.resolve(__dirname, "../" + murrix.config.filesPath) + "/" + id;
  ...
  self.getStatus = function(options, callback)
  {
    ...
    if (options.type === "image")
    {
      targetFilename = self._getDirectory() + "/" + options.id + "_" + options.width + "x" + options.height + "_"  + options.square + ".jpg";
    }
    else if (options.type === "video")
    {
      targetFilename = self._getDirectory() + "/" + id + ".webm";
    }
    else
    {
  ...
  self.clear = function(id, callback)
  {
    ...
    fs.readdir(dir, function(error, list)
    {
      ...
      for (var n = 0; n < list.length; n++)
      {
        ...
        {
          murrix.utils.removeFile(dir + "/" + targetFilename, function()
          {
            next();
          });
        });
