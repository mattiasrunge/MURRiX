...
function MurrixCacheManager(murrix)
{
  ...
  self._jobExists = function(targetFilename, callback)
  {
    ...
    {
      if (path.basename(self.runningJobs[n].options.targetFilename) === targetFilename)
      {
    ...
    self.queue.exists(targetFilename, function(error, exists)
    {
      if (error)
      {
        murrix.logger.error(self.name, "Could not check for existing job in queue, reason: " + error);
        callback(false);
        return;
      }
  ...
  self.queueItem = function(id, options, callback)
  {
    ...
    self._findItem(id, function(error, itemData)
    {
      ...
      self._jobExists(options.targetFilename, function(exists)
      {
        ...
        self.queue.push(options.targetFilename, options, function(error)
        {
          if (error)
          {
            murrix.logger.error(self.name, "Failed to queue job, reason: " + error);
            callback("Failed to queue job, reason: " + error);
            return;
          }
  ...
  self._runQueue = function()
  {
    if (murrix.server.isStarted() && !self.processingQueue)
    {
      ...
      self.queue.size(function(error, size)
      {
        if (error)
        {
          murrix.logger.error(self.name, "Failed to get size from queue, reason: " + error);
          self.processingQueue = false;
          return;
        }
        ...
        self.queue.shift(function(error, id, options)
        {
          if (error)
          {
            murrix.logger.error(self.name, "Failed to get item from queue, reason: " + error);
            return;
          }

          if (!options)
          {
            murrix.logger.info(self.name, "Queue is empty!");
            return;
          }
