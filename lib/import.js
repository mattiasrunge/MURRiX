  ...
  self.importDirectory = function(session, directory, options, callback)
  {
    ...
    fs.readdir(abspath, function(error, list)
    {
      ...
      for (var n = 0; n < list.length; n++)
      {
        ...
        else if (stat.isFile() && list[n] !== "Thumbs.db")
        {
      ...
      {
        for (var n = 0; n < directories.length; n++)
        {
          importChain.add(directories[n], function(directoryName, options2, chainCallback)
          {
      ...
      {
        importChain.add(files[n], function(file, options2, chainCallback)
        {
