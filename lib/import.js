...
function MurrixImportManager(murrix)
{
  ...
  self._createItem = function(session, id, name, filename, parentId, callback)
  {
    ...
    chain.add(null, function(data, options, chainCallback)
    {
        ...
        /*
      if (itemData.exif.GPSDateTime && itemData.exif.GPSDateTime !== "0000:00:00 00:00:00Z" && itemData.exif.GPSDOP !== 0)
      {
    ...
    chain.add(null, function(data, options, chainCallback)
    {
        var serialNumber = itemData.exif.SerialNumber || itemData.exif.InternalSerialNumber;

      if (serialNumber)
      {
        murrix.db.findOne({ type: "camera", serial: serialNumber }, "nodes", function(error, nodeData)
        {
          if (error)
          {
