...
function MurrixImportManager(murrix)
{
  ...
  {
    if (session.files && session.files[uploadId])
    {
      self.importFile(session, session.files[uploadId].name, session.files[uploadId].path, parentId, function(error, itemData)
      {
        if (error)
        {
          callback(error);
        ...
        murrix.utils.file.remove(session.files[uploadId].path);

        delete session.files[uploadId];
    ...
    {
      murrix.logger.error(self.name, "Could not find any uploaded file with id " + uploadId);
      callback("Could not find any uploaded file with id " + uploadId);
    }
  };
  ...
  self.importUploadedFileVersion = function(session, uploadId, itemId, callback)
  {
    if (session.files && session.files[uploadId])
    {
      self.importFileVersion(session, session.files[uploadId].name, session.files[uploadId].size, session.files[uploadId].path, itemId, function(error, itemData)
      {
        if (error)
        {
          callback(error);
        ...
        murrix.utils.removeFile(session.files[uploadId].path);
  ...
  self.importDirectory = function(session, directory, options, callback)
  {
    ...
    fs.readdir(abspath, function(error, list)
    {
      ...
      for (n = 0; n < list.length; n++)
      {
        ...
        else if (stat.isFile() && !murrix.utils.array.in(list[n], self.ignoreList))
        {
          files.push({ name: list[n], isRaw: false, size: stat.size });
        }
      }

      var importChain = new Chain();

      if (options.recursive)
      {
        importChain.add(directories, function(directoryName, options2, chainCallback)
        {
      ...
      importChain.addMany(files, function(file, options2, chainCallback)
      {
        murrix.utils.file.mimetype(abspath + "/" + file.name, function(error, mimetype)
        {
          if (error)
          {
            chainCallback(error);
          ...
          file.isRaw = murrix.utils.file.isRawImage(mimetype);
          chainCallback();
        });
      });

      importChain.run(function(error)
      {
        if (error)
        {
          murrix.logger.error(self.name, error);
        ...
        else
        {
          self._findOrCreateImportNode(session, abspath, options, function(error, nodeData)
          {
            ...
            chain.addMany(files, function(file, options, chainCallback)
            {
              if (!file.isRaw)
              {
  ...
  self.importFile = function(session, filename, sourceFilename, parentId, callback)
  {
    ...
    murrix.utils.file.copy(sourceFilename, murrix.config.getPathFiles() + id, function(error)
    {
      if (error)
      {
        callback(error);
      ...
      self._createItem(session, id, filename, sourceFilename, parentId, function(error, itemData)
      {
        if (error)
        {
          murrix.utils.file.remove(murrix.config.getPathFiles() + id);
          callback(error);
          return;
        }
  ...
  self.importFileVersion = function(session, filename, size, sourceFilename, itemId, callback)
  {
    murrix.db.findOne({ _id: itemId }, "items", function(error, itemData)
    {
      ...
      murrix.utils.file.md5(sourceFilename, function(error, checksum)
      {
        if (error)
        {
          callback(error);
        ...
        murrix.db.items.save(session, itemData, function(error, itemDataNew)
        {
          ...
          murrix.utils.file.copy(sourceFilename, murrix.config.getPathFiles() + versionId, function(error)
          {
            if (error)
            {
  ...
  self._createItem = function(session, id, name, filename, parentId, callback)
  {
    ...
    {
      murrix.utils.file.md5(filename, function(error, checksum)
      {
        if (error)
        {
          chainCallback(error);
    ...
    chain.add(null, function(data, options, chainCallback)
    {
      murrix.utils.file.exif(filename, function(error, exif)
      {
        if (error)
        {
          chainCallback(error);
    ...
    chain.add(null, function(data, options, chainCallback)
    {
      if (itemData.exif.GPSDateTime)
      {
        ...
        itemData.when.source.datestring = murrix.utils.time.cleanDatestring(itemData.exif.GPSDateTime);
      ...
      else if (itemData.exif.DateTimeOriginal)
      {
        ...
        itemData.when.source.datestring = murrix.utils.time.cleanDatestring(itemData.exif.DateTimeOriginal);
