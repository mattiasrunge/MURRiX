...
function MurrixUserManager(murrix)
{
  ...
  murrix.client.on("done", function()
  {
    ...
    murrix.client.register("connectUserAndGroup", function(session, args, callback)
    {
      self.connectUserAndGroup(session, args.userId, args.groupId, callback);
    });
  ...
  self.getUser = function(session, callback)
  {
    ...
    {
      if (self.user && self.user._id === session._user)
      {
      ...
      murrix.db.mongoDb.collection("users", function(error, collection)
      {
        ...
        collection.findOne({ _id: session._user }, function(error, userData)
        {
          if (error)
          {
  ...
  self.becomeUser = function(session, username, callback)
  {
    murrix.db.mongoDb.collection("users", function(error, collection)
    {
      if (error)
      ...
      collection.findOne(query, function(error, userData)
      {
        ...
        murrix.logger.info(self.name, "Became user " + userData.username + "!");
    ...
    self.getUser(session, function(error, user)
    {
      ...
      murrix.db.mongoDb.collection("users", function(error, collection)
      {
        ...
        var update = {
            ...
            modified: { timestamp: murrix.utils.timestamp(), _by: user._id }
        };

        collection.update({ _id: id }, { $set: update }, function(error, document)
        {
          if (error)
          {
            murrix.logger.error(self.name, "Could not run query");
  ...
  self.saveUser = function(session, userData, callback)
  {
    self.getUser(session, function(error, user)
    {
      ...
      if (!(user.admin || (userData._id === user._id && !!userData.admin === !!user.admin)))
      {
      ...
      murrix.db.mongoDb.collection("users", function(error, collection)
      {
        ...
        userData.modified = { timestamp: murrix.utils.timestamp(), _by: user._id };
        ...
        if (userData._id)
        {
          collection.update({ _id: userData._id }, userData, { }, function(error, numberOfUpdatedRows)
          {
            ...
            if (self.user && self.user._id === userData._id) {
                self.user = userData;
            }
  ...
  self.removeUser = function(session, id, callback)
  {
    ...
    self.connectUserAndGroup = function(session, userId, groupId, callback) {
        self.getUser(session, function(error, user) {
            if (error) {
                callback(error);
                return;
            ...
            if (!user.admin) {
                callback("No rights to connect user and group!");
                return;
            }
            ...
            murrix.db.mongoDb.collection("users", function(error, collection) {
                ...
                if (!userId) {
                    callback("Can not connect without userId!");
                    return;
                }

                if (!groupId) {
                    callback("Can not connect without groupId!");
                    return;
                }

                collection.update({ _id: userId }, { $addToSet: { _groups: groupId } }, { }, function(error, numberOfUpdatedRows) {
                    callback(error);
                });
            });
        });
    ...
    murrix.db.mongoDb.collection("users", function(error, collection)
    {
      ...
      collection.findOne({ username: "admin" }, function(error, userData)
      {
        ...
        if (userData === null)
        {
          ...
          userData.modified = { timestamp: murrix.utils.timestamp(), _by: userData._id };
          userData.admin = true;
        ...
        {
          murrix.logger.debug(self.name, "Admin user found!");
...
util.inherits(MurrixUserManager, events.EventEmitter);
