...
function User(db)
{
    ...
    if (session && session.document && session.document._user)
    {
      ...
      db.collection("users", function(error, collection)
      {
        ...
        collection.findOne({ _id: session.document._user }, function(error, userData)
        {
          ...
          console.log("Found user with _id " + session.document._user);
          self.user = userData;
  ...
  self.getUsers = function(session, callback)
  {
    self.getUser(session, function(error, user)
    {
      if (error)
      ...
      db.collection("users", function(error, collection)
      {
        ...
        collection.find({}, function(error, cursor)
        {
          ...
          cursor.toArray(function(error, userDataList)
          {
            if (error)
            {
              console.log("Could not convert results to array");
              callback(error);
              return;
            }
  ...
  self.getGroups = function(session, callback)
  {
    self.getUser(session, function(error, user)
    {
      if (error)
      ...
      db.collection("groups", function(error, collection)
      {
        ...
        collection.find({}, function(error, cursor)
        {
          ...
          cursor.toArray(function(error, groupDataList)
          {
            if (error)
            {
              console.log("Could not convert results to array");
      ...
      db.collection("groups", function(error, collection)
      {
        ...
        if (groupData._id)
        {
          collection.update({ _id: groupData._id }, groupData, { }, function(error, numberOfUpdatedRows)
          {
            if (error)
            {
              callback("Failed to update group document", null);
              return;
            }
        ...
        else
        {
          ...
          collection.insert(groupData, function(error, groupData)
          {
            if (error)
            {
              callback("Failed to insert group document", null);
              return;
            }
      ...
      db.collection("users", function(error, collection)
      {
        ...
        if (userData._id)
        {
          collection.update({ _id: userData._id }, userData, { }, function(error, numberOfUpdatedRows)
          {
            if (error)
            {
              callback("Failed to update user document", null);
              return;
            }
        ...
        else
        {
          ...
          collection.insert(userData, function(error, userData)
          {
            if (error)
            {
              callback("Failed to insert user document", null);
              return;
            }
  ...
  self.removeGroup = function(session, id, callback)
  {
    self.getUser(session, function(error, user)
    {
      ...
      db.collection("groups", function(error, collection)
      {
        ...
        collection.remove({ _id: _id }, function(error)
        {
          if (error)
          {
            callback("Failed to delete group document", null);
            return;
          }
    ...
    self.getUser(session, function(error, user)
    {
      ...
      db.collection("users", function(error, collection)
      {
        ...
        collection.remove({ _id: _id }, function(error)
        {
          if (error)
          {
            callback("Failed to delete user document", null);
            return;
          }
