...
module.exports = function()
{
  ...
  {
    request.session = sessions.get(request.cookies[sessions.name()]) || sessions.create();

    response.on("header", function()
    {
      response.cookie(sessions.name(), request.session._id);
    });
  ...
  {
    response.send(clients.getApiSpec());
  });
...
/*
  ...
  io.configure(function()
  {
    io.set("authorization", function(handshakeData, callback)
    {
      ...
      var session = sessions.get(cookies[sessions.name()]);

      if (!session)
      {
        logger.error("No session found for id " + cookies[sessions.name()]);
        callback(null, false);
        return;
      }
      ...
      try {
        userinfo = JSON.parse(unescape(cookies.userinfo));
      } catch (e) {
        logger.error("Could not parse userinfo", cookies.userinfo);
      }
      
      if (userinfo && !user.isLoggedIn(session))
      {
