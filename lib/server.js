  ...
  self.start = function()
  {
    ...
    function httpRequestHandler(request, response)
    {
      ...
      murrix.session.start(request, response, function(error, session)
      {
        ...
        else if (requestParams.pathname === "/gps")
        {
          var position = murrix.utils.parseNmea(requestParams.query.gprmc);

          if (position === false)
          {
            murrix.logger.error(self.name, "Could not parse NMEA:" + requestParams.query.gprmc);
          }
          else if (!position.valid)
          {
            murrix.logger.info(self.name, "Received position is not valid:" + requestParams.query.gprmc);
          }
          else
          {
            position.altitude = parseFloat(requestParams.query.alt);
            position.source = "gps";
            ...
            murrix.user.becomeUser(session, "admin", function(error)
            {
              ...
              murrix.db.findOne({ tracker_id: requestParams.query.id }, "nodes", function(error, trackerNodeData)
              {
                if (error)
                {
                  murrix.logger.error("Could not find node with tracker id " + requestParams.query.id + ", error: " + error);
                  return;
                }
