...
function MurrixDatabaseManager(murrix)
{
  ...
  murrix.config.on("done", function()
  {
    ...
    self.mongoDb.open(function(error, mongoDb)
    {
      if (error)
      {
        murrix.logger.error(self.name, "Failed to connect to mongoDB, reason: " + error.toString());
        exit(1);
        return;
      }
  ...
  self.error = function(error, callback, prefix)
  {
      ...
      else
      {
        ...
        murrix.triggers.triggerPreSave(triggerEvent, {}, data, function(error, dataNew)
        {
          ...
          collection.insert(dataNew, function(error, dataInserted)
          {
            ...
            murrix.triggers.triggerPostSave(triggerEvent, {}, dataInserted[0], function(error)
            {
              if (error)
              {
                self.error(error, callback);
  ...
  self._filterItemsWithRights = function(session, results, callback)
  {
    var parents = [];

    for (var n = 0; n < results.length; n++)
    {
      parents = murrix.utils.array.add(results[n]._parents, parents);
    }

    self.findWithRights(session, { _id: { $in: parents } }, { collection: "nodes", fields: { _id: true } }, function(error, nodeList)
    {
      if (error)
      {
        self.error(error, callback, "Could not get parent nodes to items, reason: ");
        return;
      }
      ...
      for (var n = 0; n < results.length; n++)
      {
        if (murrix.utils.array.intersect(nodeIds, results[n]._parents).length > 0)
        {
          filteredResult.push(results[n]);
        }
      }
  ...
  self.findWithRights = function(session, query, options, callback)
  {
    ...
    else if (collectionName === "items")
    {
      if (options.fields)
      {
        options.fields._parents = true; // We need this to do rights checks
      }
      ...
      self.find(query, options, function(error, results)
      {
        ...
        self._filterItemsWithRights(session, results, callback);
      });
    ...
    else if (collectionName === "triggers")
    {
      murrix.user.getUser(session, function(error, user)
      {
        if (error)
        {
          self.error(error, callback, "Could not get user, reason: ");
          return;
        }
  ...
  self.findOneWithRights = function(session, query, options, callback)
  {
    self.findWithRights(session, query, options, function(error, dataList)
    {
      ...
      {
        self.error("Could not find any node", callback);
        return;
      }
  ...
  self.countWithRights = function(session, query, options, callback)
  {
    ...
    {
      options.fields = { _id: true, _parents: true };

      self.findWithRights(session, query, options, function(error, results)
      {
        if (error)
        {
          self.error(error, callback);
  ...
  self.nodes.findRandom = function(session, options, callback)
  {
    murrix.user.getUser(session, function(error, userNode)
    {
      ...
      self.nodes.addAccessCheckToQuery(session, { type: "album" }, false, function(error, query)
      {
  ...
  self.nodes.save = function(session, nodeData, callback)
  {
    murrix.user.getUser(session, function(error, userNode)
    {
      ...
      nodeData.added = nodeData.added || { timestamp: murrix.utils.time.timestamp(), _by: userNode._id };
      nodeData.modified = { timestamp: murrix.utils.time.timestamp(), _by: userNode._id };
    ...
    murrix.user.getUser(session, function(error, userNode)
    {
      ...
      self.nodes.addAccessCheckToQuery(session, { _id: nodeId }, false, function(error, query)
      {
        ...
        self.findOne(query, "nodes", function(error, nodeData)
        {
          ...
          comment.added = { timestamp: murrix.utils.time.timestamp(), _by: userNode._id };
          comment.text = text;
      ...
      self.nodes.addAccessCheckToQuery(session, { }, false, function(error, query)
      {
        ...
        self.find(query, "items", function(error, itemDataList)
        {
          ...
          for (var key in itemDataList)
          {
            ...
            {
              if (!murrix.utils.array.in(itemDataList[key]._parents[n], parentIdList))
              {
    ...
    murrix.user.getUser(session, function(error, userNode)
    {
      ...
      self.findOne({ _id: itemId }, "items", function(error, itemData)
      {
        ...
        self.nodes.addAccessCheckToQuery(session, { _id: { $in: itemData._parents } }, true, function(error, query)
        {
          if (error)
          {
            self.error(error, callback, "Could not add access checks to query, reason: ");
            return;
          }

          if (query === false)
          {
            callback("No rights on parents, can not remove item!");
            return;
          }
  ...
  self.items.save = function(session, itemData, callback)
  {
    murrix.user.getUser(session, function(error, userNode)
    {
      ...
      // Verify attributes for all types
      itemData.added = itemData.added || { timestamp: murrix.utils.time.timestamp(), _by: userNode._id };
      itemData.modified = { timestamp: murrix.utils.time.timestamp(), _by: userNode._id };
      ...
      self.findOne({ _id: itemData._id }, "items", function(error, itemDataDb)
      {
        ...
        // TODO: Evaulate if this is actually 100%, maybe merge parent lists?
        ...
        self.nodes.addAccessCheckToQuery(session, { _id: { $in: parents } }, true, function(error, query)
        {
          ...
          if (query === false)
          {
            callback("No rights on parents, can not change/create item!");
            return;
          }
  ...
  // TODO: Deprecate this function and use the functions in helpers instead
  ...
  self.items.importFile = function(session, name, uploadId, parentId, callback)
  {
    murrix.upload.find(session, uploadId, function(error, file)
    {
      if (error)
      {
        self.error(error, callback, "Could not find uploaded file with id " + uploadId + ", reason: ");
        return;
      }
      ...
      {
        source.type = "gps";
        source.datestring = murrix.utils.time.cleanDatestring(file.exif.GPSDateTime);
      ...
      {
        source.type = "camera";
        source.datestring = murrix.utils.time.cleanDatestring(file.exif.DateTimeOriginal);
      ...
      if (file.exif.SerialNumber)
      {
        chain.add(function(args, options, next)
        {
          self.findOne({ type: "camera", serial: file.exif.SerialNumber }, "nodes", function(error, nodeData)
          {
            if (error)
            {
              next("Failed to get camera, reason: " + error);
              return;
            }
