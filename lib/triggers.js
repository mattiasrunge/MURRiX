  ...
  self._chainClearCache = function(options, next)
  {
    ...
    if (options.event === self.EVENT_ITEM_REMOVE)
    {
      ...
      clear = true;
             ...
             options.dataOld.thumbPosition !== options.dataNew.thumbPosition)
    {
      murrix.logger.info(self.name, "Angle, mirror or thumbPosition has changed on item, will clear cache and set a new cache id!");

      if (options.dataOld.angle === options.dataNew.angle &&
          options.dataOld.mirror === options.dataNew.mirror &&
          options.dataOld.thumbPosition !== options.dataNew.thumbPosition)
      {
        murrix.logger.debug(self.name, "Only thumbPosition has changed, will not clear video cache only image cache!");
        excludeFilter.push("video");
      }
    ...
    if (clear)
    {
      murrix.cache.clear(options.dataOld._id, excludeFilter, function(error)
      {
        if (error)
        {
  ...
  self._chainItemUpdateTimestamp = function(options, next)
  {
        ...
        JSON.stringify(options.dataOld.when) !== JSON.stringify(options.dataNew.when))
    {
      ...
      murrix.db.findOne({ _id: options.dataNew._with }, "nodes", function(error, nodeData)
      {
        ...
        if (options.dataNew.when.source && options.dataNew.when.source.type === "gps") {

            // TODO: If GPS data is the same as first prvious with GPS data, use camera and remove where!!!
        }
        /*
  ...
  self._chainCameraUpdateItemTimestamp = function(options, next)
  {
        ...
        JSON.stringify(options.dataOld.referenceTimelines) !== JSON.stringify(options.dataNew.referenceTimelines))
    {
      ...
      murrix.db.find({ _with: options.dataNew._id }, "items", function(error, itemDataList)
      {
        ...
        murrix.logger.debug(self.name, "Found " + itemDataList.length + " items to update timestamps for!");

        var chain = new MurrixChain();

        for (var n = 0; n < itemDataList.length; n++)
        {
          chain.add(itemDataList[n], function(itemData, options2, chainCallback)
          {
            ...
            murrix.logger.debug(self.name, "Updating timestamp for " + itemData._id);
            ...
            murrix.db.save(itemData, "items", false, function(error)
            {
              if (error)
              {
                ...
                murrix.logger.error(self.name, "Failed to save item, reason: " + error);
        ...
        chain.final(function(error)
        {
          ...
          murrix.logger.debug(self.name, "All done updating timestamps");
