...
function MurrixTriggerManager(murrix, userTriggersFile)
{
  ...
  self._chainItemUpdatePartner = function(options, next)
  {
    ...
    if (options.dataOld.family._partner !== options.dataNew.family._partner)
    {
      var itemChain = stewardess();

      if (options.dataOld.family && options.dataOld.family._partner)
      {
        itemChain.add(function(options2, next2)
        {
      ...
      if (options.dataNew.family && options.dataNew.family._partner)
      {
        itemChain.add(function(options2, next2)
        {
  ...
  self._createChain = function(callback)
  {
    ...
    // Rotate showing if angle changes
  ...
  self.trigger = function(event, dataOld, dataNew, callback)
  {
    if (event === false)
    {
      return;
    ...
    var chain = self._createChain(callback);
    var options = {};
