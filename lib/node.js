    ...
    user.getUser(session, function(error, user)
    {
      ...
      if (user)
      {
        if (!user.user.admin)
        {
          if (adminRequired)
          {
            query.$or = [ { "added._by": user._id }, { _id: user._id } ]; // , { _admins: { $in: user.user._groups } }
          }
          else
          {
            query.$or = [ { public: true }, { "added._by": user._id }, { _id: user._id } ]; // , { _readers: { $in: user.user._groups } }
          }
