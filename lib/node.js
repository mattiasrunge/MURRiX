...
function NodeManager(db, user)
{
    ...
    user.getUser(session, function(error, userNode)
    {
      ...
      if (nodeData._id)
      {
        ...
        self.addAccessCheckToQuery(session, { _id: nodeData._id }, true, function(error, query)
        {
          ...
          self._count(query, "nodes", function(error, count)
          {
            if (error)
            {
              console.log(error);
  ...
  self.updateTriggers = function(triggersFile)
  {
    if (fs.existsSync(triggersFile))
    {
      console.log("Found triggers file at " + triggersFile);

      var triggers = require(triggersFile);

      db.collection("triggers", function(error, collection)
      {
        if (error)
        {
          console.log(error);
        ...
        {
          console.log("Found trigger: " + triggers[n].name);
    ...
    {
      console.log("No file with triggers found, no update will be performed");
    }
  };

  self._runTrigger = function(event, data, callback)
  {
    self._find({ event: event }, "triggers", function(error, triggerList)
    {
      if (error)
      {
        callback(error);
      ...
      for (var id in triggerList)
      {
        ...
        for (var n = 0; n < trigger.criterias.length; n++)
        {
          ...
          {
            if (data[trigger.criterias[n].key] !== trigger.criterias[n].value)
            {
              matched = false;
              break;
            }
          ...
          {
            if (!MurrixUtils.inArray(trigger.criterias[n].value, data[trigger.criterias[n].key]))
            {
              matched = false;
              break;
            }
          ...
          {
            console.log("Unknown match found on criteria");
            console.log(trigger.criterias[n]);
            matched = false;
          }
        }
        ...
        if (matched)
        {
          for (var n = 0; n < trigger.actions.length; n++)
          {
            ...
            {
              data[trigger.actions[n].key] = trigger.actions[n].value;
            }
            else if (trigger.actions[n].action === "push")
            {
              data[trigger.actions[n].key] = data[trigger.actions[n].key] || [];
              data[trigger.actions[n].key].push(trigger.actions[n].value);
            }
          }
        }
      }
  ...
  self._saveNode = function(nodeData, callback)
  {
    ...
    db.collection("nodes", function(error, collection)
    {
      ...
      if (nodeData._id)
      {
        self._runTrigger("updateNode", nodeData, function(error, nodeData)
        {
      ...
      else
      {
        ...
        self._runTrigger("createNode", nodeData, function(error, nodeData)
        {
  ...
  self._saveItem = function(itemData, callback)
  {
    ...
    db.collection("items", function(error, collection)
    {
      ...
      self.updateItemWhen(itemData, function(error, newItemData)
      {
        ...
        if (newItemData._id)
        {
          self._runTrigger("updateItem", newItemData, function(error, newItemData)
          {
        ...
        else
        {
          ...
          self._runTrigger("createItem", newItemData, function(error, newItemData)
          {
...
//             itemData.angle = 0;
  ...
  self.hideRaw = function(session, rawItemData, callback)
  {
    var name = path.basename(rawItemData.name, path.extname(rawItemData.name));

    var query = {};

    query.$and = [ { name: { $regex: name + ".*", $options: "-i" } }, { name: { $ne: rawItemData.name } } ];
    query._parents = rawItemData._parents;
    ...
    self.find(session, query, "items", function(error, itemDataList)
    {
      ...
      itemDataList = MurrixUtils.makeArray(itemDataList);
      ...
      itemData.versions.push({ id: rawItemData._id, name: rawItemData.name, size: rawItemData.exif.FileSize });

      console.log("Adding raw version to " + itemData.name + ", id " + itemData._id);
      console.log(JSON.stringify(itemData.versions));

      self.saveItem(session, itemData, function(error, itemDataNew)
      {
        if (error)
        {
          callback(error);
        ...
        db.collection("items", function(error, collection)
        {
          ...
          collection.remove({ _id: rawItemData._id }, function(error, removed)
          {
            if (error)
            {
              callback(error);
  ...
  self._findOrCount = function(session, query, options, fn, callback)
  {
    var collectionName = (typeof options === "string" ? options : options.collection);

    if (collectionName === "items")
    {
      fn(query, options, callback);
      return;
    }
  ...
  {
    self._findOrCount(session, query, options, self._find, callback);
  };

  self.count = function(session, query, options, callback)
  {
    self._findOrCount(session, query, options, self._count, callback);
  };
  ...
  self._count = function(query, options, callback)
  {
    var collectionName = typeof options === "string" ? options : options.collection;
