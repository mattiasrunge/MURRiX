...
function NodeManager(db, user)
{
  ...
  self.allowedTypes = [ "event", "person", "location", "thing", "file" ];
  ...
  self.save = function(session, nodeData, callback)
  {
    user.getUser(session, function(error, userNode)
    {
      ...
      nodeData.addedDatetime = nodeData.addedDatetime || MurrixUtils.timestamp();
      nodeData.modifiedDatetime = MurrixUtils.timestamp();
      ...
      switch (nodeData.type)
      {
        ...
        case "thing":
        {
          nodeData._owners = nodeData._owners || [];
          nodeData._profilePicture = nodeData._profilePicture || false;
          break;
        }
      }
  ...
  self.createFile = function(session, uploadManager, name, uploadId, parentId, callback)
  {
    uploadManager.find(session, uploadId, function(error, file)
    {
      ...
      if (file.exif.GPSDateTime)
      {
        ...
        nodeData.createdDatetime = { datetime: timestamp, source: "gps" }; // TODO: Fix date and time format and time zone
      }
      else if (file.exif.DateTimeOriginal)
      {
        ...
        var timestamp = MurrixUtils.timestamp(datetime);
        nodeData.createdDatetime = { datetime: timestamp, source: "exif" }; // TODO: Fix date and time format
      }
      ...
      if (file.exif.GPSDateTime)
      {
        ...
        nodeData.position.altitude = file.exif.GPSAltitude || 0;
        nodeData.position.type = file.exif.GPSAltitude ? "3D": "2D";
        nodeData.position.longitude = file.exif.GPSLongitude;
        nodeData.position.latitude = file.exif.GPSLatitude;
        nodeData.position.source = "gps";
      }
      ...
      self.save(session, nodeData, function(error, nodeData)
      {
        ...
        self.find(session, { _id: nodeData._parentNode }, function(error, nodeDataList)
        {
          ...
          uploadManager.moveFile(session, file.id, nodeData._id, function(error)
          {
            if (error)
            {
              callback(error);
    ...
    user.getUser(session, function(error, userNode)
    {
      ...
      self.count(session, { _id: nodeId }, function(error, count)
      {
        ...
        db.collection("nodes", function(error, collection)
        {
          ...
          comment.addedDatetime = MurrixUtils.timestamp();
          comment._addedBy = userNode ? userNode._id : false;
          comment.text = text;

          collection.update({ _id: nodeId }, { $set: { modifiedDatetime: MurrixUtils.timestamp() }, "$push" : { comments: comment } }, function(error, numberOfUpdatedRows)
          {
            if (error)
            {
              callback("Failed to update document", null);
            ...
            self.find(session, { _id: nodeId }, function(error, nodeDataList)
            {
              if (error)
              {
                callback(error);
              ...
              console.log("commentsafter", nodeDataList[nodeId].comments);
    ...
    user.getUser(session, function(error, user)
    {
      ...
      if (user)
      {
        if (!user.user.admin)
        {
          if (adminRequired)
          {
            query.$or = [ { _addedBy: user._id }, { _id: user._id } ]; // , { _adminAccess: { $in: user.user._groups } }
          }
          else
          {
            query.$or = [ { publicAccess: true }, { _addedBy: user._id }, { _id: user._id } ]; // , { _readAccess: { $in: user.user._groups } }
          }
