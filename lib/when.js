...
when = function()
{
  ...
  self.updateWhenRecursive = function(nodeManager, itemDataList, references, callback)
  {
    ...
    self.updateWhen(itemData.when, references, function(error, when)
    {
      if (error)
      {
      ...
      if (when.timestamp != itemData.when.timestamp)
      {
        nodeManager._saveItem(itemData, function(error, newItemData)
        {
          ...
          self.updateWhenRecursive(nodeManager, itemDataList, references, callback);
        });
      }
      else
      {
  ...
  self.updateItemWhen = function(nodeManager, itemData, callback)
  {
    ...
    {
      // Find camera
      nodeManager._findOne({ _id: itemData._with }, "nodes", function(error, nodeData)
      {
        if (error)
        {
          callback(error);
  ...
  self.cameraUpdated = function(nodeManager, cameraNodeData, callback)
  {
    // Find all items with camera as _with
    nodeManager._find({ _with: cameraNodeData._id }, "items", function(error, itemDataList)
    {
      if (error)
      {
        callback(error);
      ...
      self.updateWhenRecursive(nodeManager, list, cameraNodeData.referenceTimelines, function(error)
      {
        if (error)
        {
          callback(error);
  ...
  {
    // Find camera
    nodeManager._findOne({ _id: id }, "nodes", function(error, nodeData)
    {
      if (error)
      {
        callback(error);
      ...
      nodeManager._saveNode(nodeData, function(error, newNodeData)
      {
        ...
        self.cameraUpdated(nodeManager, newNodeData, function(error)
        {
          if (error)
          {
            callback(error);
