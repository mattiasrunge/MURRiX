...
exports.save = function(session, args, callback)
{
  user.current(session, function(error, userData)
  {
    ...
    args.added = args.added || { timestamp: utils.time.timestamp(), _by: userData._id };
    args.modified = { timestamp: utils.time.timestamp(), _by: userData._id };
    args.description = args.description || "";
    args.comments = args.comments || [];
    args._admins = args._admins || [];
    args._readers = args._readers || [];
    args["public"] = args["public"] || false;
    args.tags = args.tags || [];
    args.removed = args.removed || false;
    args._profilePicture = args._profilePicture || false;
    
    if (args._id)
    {
      exports.count(session, { query: { _id: args._id }, limit: 1 }, function(error, count)
      {
        if (error)
        {
          callback(error);
