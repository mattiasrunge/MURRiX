...
function MurrixHelpersManager(murrix)
{
  ...
  murrix.client.on("done", function()
  {
    murrix.client.register("helper_nodeGetFilesList", function(session, args, callback)
    {
      ...
      murrix.db.findOneWithRights(session, { _id: args.nodeId }, "nodes", function(error, nodeData)
      {
        if (error)
        {
          callback(error);
        ...
        murrix.db.findWithRights(session, query, "items", function(error, itemDataList)
        {
          if (error)
          {
            callback(error);
