...
function MurrixHelpersManager(murrix)
{
  ...
  function getValueByPath(obj, paths) {
      ...
      return getValueByPath(obj[p], paths);
  };

  var types = { "n" : "nodes", "i" : "items", "u" : "users", "g" : "groups" };
  function resolvePath(session, value, paths) {
      var p = paths.shift();
      var parts = p.split(":");
      var type = parts[0];
      var id = parts[1].replace(/\$/g, value);
      var attr = parts[2];

      return new Promise(function(resolve, reject) {
          var options = { collection: types[type] };
          var fn = null;

          if (type === "u" || type === "g" || type === "i") {
              fn = murrix.db.findOne.bind(murrix.db);
          } else if (type === "n") {
              fn = murrix.db.findOneWithRights.bind(murrix.db, session);
          }
console.log(type, id, attr, options);
          fn({ _id: id }, options, function(error, nodeData) {
              if (error) {
                  return reject(error);
              }

              if (!nodeData) {
                  return reject("Could not resolve: " + p + ", value: " + value);
              }

              value = attr ? getValueByPath(nodeData, attr.split(".")) : nodeData;

              if (paths.length > 0) {
                  return resolve(resolvePath(session, value, paths));
              }
  ...
  {
      murrix.client.register("helper_resolve", function(session, args, callback) {
          resolvePath(session, null, args.split("|"))
          .then(function(value) {
              callback(null, value);
          })
          .catch(callback);
    ...
    murrix.client.register("helper_nodeToolsReReadExif", function(session, args, callback) {
        murrix.db.findOne({ _id: args.id }, "items", function(error, itemData) {
            if (error) {
                callback(error);
                return;
            }

            murrix.utils.itemReadExif(itemData);
    ...
    murrix.client.register("helper_nodeToolsGetCameraList", function(session, args, callback)
    {
      ...
      murrix.db.findWithRights(session, { _parents: args.nodeId, what: "file" }, options, function(error, itemDataList)
      {
        ...
        for (var n = 0; n < itemDataList.length; n++)
        {
          ...
          if (item.exif.SerialNumber || item.exif.InternalSerialNumber)
          {
            identifier.type = "serial";
            identifier.unique = (item.exif.SerialNumber || item.exif.InternalSerialNumber).replace(/\u0000/g, "");
          }
      ...
      murrix.db.findOneWithRights(session, { _id: args.nodeId }, options, function(error, nodeData)
      {
        ...
        self.getItemQuery(session, nodeData, [ "file" ], function(error, query)
        {
          ...
          murrix.db.findWithRights(session, query, options, function(error, itemDataList)
          {
            ...
            var list = [];
            var id = murrix.utils.timestamp();
            var filename = murrix.config.getPathTemp() + id + "_" + args.nodeId + ".zip";

            for (var n = 0; n < itemDataList.length; n++)
            {
              list.push({ name: murrix.utils.lpad(n + '', "0", 4) + "_" + itemDataList[n].name, path: murrix.config.getPathFiles() + itemDataList[n]._id });
            }
