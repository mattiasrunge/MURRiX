...
function MurrixHelpersManager(murrix)
{
  ...
  murrix.client.on("done", function()
  {
        ...
        murrix.db.findOneWithRights(session, { _id: id, type: "person" }, options, function(error, nodeData)
        {
          ...
          chain.run(function(error)
          {
            ...
            person.children.sort(function(a, b)
            {
              ...
              return parseInt(murrix.utils.string.rpad(a.birth.replace(/-/g, ""), "0", 8), 10) - parseInt(murrix.utils.string.rpad(b.birth.replace(/-/g, ""), "0", 8), 10);
            });
    ...
    murrix.client.register("helper_nodeGetTimelineList", function(session, args, callback)
    {
      ...
      murrix.db.findOneWithRights(session, { _id: args.nodeId }, options, function(error, nodeData)
      {
        ...
        self.getItemQuery(session, nodeData, [ "file", "text" ], function(error, query)
        {
          ...
          var options = { collection: "items", fields: { _id: true, tagSearch: true, when: true, what: true, _who: true, where: true, name: true, text: true, cacheId: true, "exif.MIMEType": true } };
          ...
          murrix.db.findWithRights(session, query, options, function(error, itemDataList)
          {
            ...
            for (var n = 0; n < itemDataList.length; n++)
            {
              ...
              else
              {
                ...
                var datestampString = murrix.utils.string.lpad(datestamp[0] + "", "0", 4);
                datestampString += murrix.utils.string.lpad(datestamp[1] + "", "0", 2);
                datestampString += murrix.utils.string.lpad(datestamp[2] + "", "0", 2);
              ...
              {
                if (murrix.utils.file.isImage(itemData.exif.MIMEType) || murrix.utils.file.isVideo(itemData.exif.MIMEType))
                {
                  results[datestampCompare].images.push({ _id: itemData._id, name: itemData.name, when: itemData.when, cacheId: itemData.cacheId, type: itemData.exif ? itemData.exif.MIMEType : null });
                }
                else if (murrix.utils.file.isAudio(itemData.exif.MIMEType))
                {
                  results[datestampCompare].audio.push({ _id: itemData._id, name: itemData.name, when: itemData.when, type: itemData.exif ? itemData.exif.MIMEType : null });
                }
              }
            }
    ...
    murrix.client.register("helper_nodeToolsRemoveDuplicates", function(session, args, callback)
    {
      ...
      chain.addMany(args.list, function(itemData, chainOptions, chainCallback)
      {
    ...
    murrix.client.register("helper_nodeToolsHideRaw", function(session, args, callback)
    {
      ...
      chain.addMany(args.list, function(itemDataRaw, chainOptions, chainCallback)
      {
      ...
      chain.run({ itemDataList: [] }, function(error, options)
      {
        if (error)
        {
    ...
    murrix.client.register("helper_nodeToolsGetHideRawList", function(session, args, callback)
    {
      ...
      murrix.db.findWithRights(session, { _parents: args.nodeId, what: "file", "exif.MIMEType": { $in: murrix.utils.file.mimetypes["rawimage"] } }, options, function(error, itemDataList)
      {
        if (error)
        {
        ...
        chain.addMany(itemDataList, function(itemDataRaw, chainOptions, chainCallback)
        {
        ...
        chain.run({ cannotHideList: [], canHideList: [] }, function(error, chainOptions)
        {
          if (error)
          {
            murrix.logger.error(self.name, error);
    ...
    murrix.client.register("helper_nodeToolsGetCameraList", function(session, args, callback)
    {
      ...
      murrix.db.findWithRights(session, { _parents: args.nodeId, what: "file" }, options, function(error, itemDataList)
      {
        ...
        identifiers = murrix.utils.array.values(identifiers);
          ...
          murrix.db.findWithRights(session, query, options, function(error, itemDataList)
          {
            ...
            for (var n = 0; n < itemDataList.length; n++)
            {
              if (itemDataList[n].showing)
              {
                ...
                {
                  if (!murrix.utils.array.in(itemDataList[n].showing[i]._id, list))
                  {
        ...
        self.getItemQuery(session, nodeData, [ "file", "text" ], function(error, query)
        {
          ...
          murrix.db.findWithRights(session, query, options, function(error, itemDataList)
          {
            ...
            for (var n = 0; n < itemDataList.length; n++)
            {
              ...
              {
                if (!murrix.utils.array.in(itemDataList[n]._who, list))
                {
        ...
        self.getItemQuery(session, nodeData, [ "file", "text" ], function(error, query)
        {
          ...
          murrix.db.findWithRights(session, query, options, function(error, itemDataList)
          {
            ...
            for (var n = 0; n < itemDataList.length; n++)
            {
              ...
              {
                if (!murrix.utils.array.in(itemDataList[n]._with, list))
                {
        ...
        self.getItemQuery(session, nodeData, [ "file", "text" ], function(error, query)
        {
          ...
          murrix.db.findWithRights(session, query, options, function(error, itemDataList)
          {
            ...
            for (var n = 0; n < itemDataList.length; n++)
            {
              ...
              {
                if (!murrix.utils.array.in(itemDataList[n].where._id, list))
                {
    ...
    murrix.client.register("helper_nodeGetAge", function(session, args, callback)
    {
      ...
      murrix.db.findOneWithRights(session, { _id: args.nodeId }, options, function(error, nodeData)
      {
        ...
        murrix.db.findWithRights(session, query, options, function(error, itemDataList)
        {
          ...
          {
            age.ageNow = murrix.utils.time.calculateAge(birthItem.when.timestamp);
            age.birthTimestamp = birthItem.when.timestamp;

            if (deathItem && deathItem.when && deathItem.when.timestamp)
            {
              age.ageAtDeath = murrix.utils.time.calculateAge(birthItem.when.timestamp, deathItem.when.timestamp);
    ...
    murrix.client.register("helper_nodePrepareDownload", function(session, args, callback)
    {
      var options = { collection: "nodes", fields: { _id: true, name: true, type: true } };
      ...
      murrix.db.findOneWithRights(session, { _id: args.nodeId }, options, function(error, nodeData)
      {
        ...
        self.getItemQuery(session, nodeData, [ "file" ], function(error, query)
        {
          ...
          var options = { collection: "items", fields: { _id: true, name: true, when: true } };
          ...
          murrix.db.findWithRights(session, query, options, function(error, itemDataList)
          {
            ...
            var list = [];
            var id = murrix.utils.time.timestamp();
            var filename = murrix.config.getPathTemp() + id + "_" + args.nodeId + ".zip";

            for (var n = 0; n < itemDataList.length; n++)
            {
              list.push({ name: murrix.utils.string.lpad(n + '', "0", 4) + "_" + itemDataList[n].name, path: murrix.config.getPathFiles() + itemDataList[n]._id });
            }

            murrix.logger.info(self.name, "Starting to create node archive with " + list.length + " file(s)...");

            murrix.utils.createArchive(filename, list, function(error)
            {
              if (error)
              {
                murrix.logger.error(self.name, "Failed to create zip file in " + filename + ", reason: " + error);
                callback(error);
                return;
              }

              murrix.logger.info(self.name, "Archive created successfully, " + filename);

              session.archives = session.archives || {};
              session.archives[id] = { name: nodeData.name + ".zip", archive: filename };
  ...
  murrix.client.register("helper_nodeSearch", function(session, args, callback)
  {
    var query = {};
    var action = "name";
    ...
    murrix.db.countWithRights(session, query, "nodes", function(error, count)
    {
      if (error)
      {
        callback(error);
      ...
      var result = { count: count, nodeDataList: [] };
      var pages = Math.ceil(count / args.limit);
      var skip = args.limit * args.page;
      var options = { collection: "nodes", limit: args.limit, skip: skip };
      ...
      murrix.db.findWithRights(session, query, options, function(error, nodeDataList)
      {
        ...
        result.nodeDataList = nodeDataList;
  ...
  murrix.client.register("helper_getTags", function(session, args, callback)
  {
    var reduce = "function(doc, prev) { for (var n in doc.tags) { prev[doc.tags[n]] = prev[doc.tags[n]] || 0; prev[doc.tags[n]]++; } }";

    murrix.db.group({}, { collection: "nodes", reduceFunction: reduce }, function(error, result)
    {
      if (error)
      {
        callback(error);
      ...
      for (tag in result[0])
      {
        var size = minSize + (result[0][tag] / max) * (maxSize - minSize);
