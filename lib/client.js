...
namespaces.item = require("./client/item");
...
exports.getApiSpec = function() {
  var api = [];
  
  for (var namespace in namespaces) {
    for (var name in namespaces[namespace]) {
      if (name !== "event") {
        var f = namespaces[namespace][name].toString();
      
        if (f.length > 0) {
          api.push("Func:  " + namespace + "." + name + "()");
        }
      } else {
        for (var event in namespaces[namespace].event) {
          api.push("Event: " + namespace + "." + event);
        }
      }
    }
  }
